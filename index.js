window.onload=function(){
    activityDropdown();
    const activityOption = document.getElementById("listCategories");
    activityOption.onchange = onCategoryOptionChange;
}
...
function activityDropdown(){
    const activityOption = document.getElementById("listCategories")
    let categoriesNum = categories.length
    for (let i = 0 ; i < categoriesNum; i++ ){
        let theOption = new Option(categories[i]);

        activityOption.appendChild(theOption);

    };
};

    function onCategoryOptionChange( ){

    const listCategories=document.getElementById("listCategories"); 
    let listCategoriesValue = listCategories.value;
   
    let tableBody= document.getElementById("tableBody");

        for (let Options of activities){
            if (listCategoriesValue == Options.category){
                tableBodyValue(tableBody , Options);
    ...
    function tableBodyValue(tbody ,Options){
        let row = tbody.insertRow(0);
        let idCell = row.insertCell(0);
        idCell.innerHTML= Options.id;

        let nameCell = row.insertCell(1);
        nameCell.innerHTML= Options.name;

        let descriptionCell = row.insertCell(2);
        descriptionCell.innerHTML= Options.description;

        let locationCell = row.insertCell(3);
        locationCell.innerHTML=  Options.location;

        let priceCell = row.insertCell(4);
        priceCell.innerHTML=  Options.price;
